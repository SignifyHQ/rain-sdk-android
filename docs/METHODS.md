...
| `sendLoginCode` | `suspend fun sendLoginCode(email: String)` | The email channel: `sendLoginCode(RainWalletContact.Email(email))`. It sends any string as an email address, so route a phone number through `RainWalletContact.Phone`. Same codes as the contact overload: `RAIN_102`, `RAIN_502`. |
