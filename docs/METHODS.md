...
|--------|-----------|-------|
| `sendLoginCode` | `suspend fun sendLoginCode(email: String)` | Sends a one-time code by email. Applies the one-shot Turnkey configuration when it is the first auth call; touches no existing session. Throws `RainError.InvalidConfig` for a blank email. |
